...
public class DialogPerhitunganAHP extends javax.swing.JDialog {
        ...
        protected AHPCalculation ahpCalc = new AHPCalculation();
        ...
        public void getMatriksNorK() {
                double[][] normalized = ahpCalc.getCriteriaNormalized();
                double[] weights = ahpCalc.getCriteriaWeights();

                k1k1N.setText(df.format(normalized[0][0]));
                k1k2N.setText(df.format(normalized[0][1]));
                k1k3N.setText(df.format(normalized[0][2]));
                k1k4N.setText(df.format(normalized[0][3]));
                k2k1N.setText(df.format(normalized[1][0]));
                k2k2N.setText(df.format(normalized[1][1]));
                k2k3N.setText(df.format(normalized[1][2]));
                k2k4N.setText(df.format(normalized[1][3]));
                k3k1N.setText(df.format(normalized[2][0]));
                k3k2N.setText(df.format(normalized[2][1]));
                k3k3N.setText(df.format(normalized[2][2]));
                k3k4N.setText(df.format(normalized[2][3]));
                k4k1N.setText(df.format(normalized[3][0]));
                k4k2N.setText(df.format(normalized[3][1]));
                k4k3N.setText(df.format(normalized[3][2]));
                k4k4N.setText(df.format(normalized[3][3]));

                Prior1.setText(df.format(weights[0]));
                Prior2.setText(df.format(weights[1]));
                Prior3.setText(df.format(weights[2]));
                Prior4.setText(df.format(weights[3]));
        ...
        public void getPenilaian() {
                ...
                if (priceIndex >= 0) {
                        double hargaContribution = subcriteriaWeights[0][priceIndex];
                        totalNilai += hargaContribution;
                }

                if (ccIndex >= 0) {
                        double ccContribution = subcriteriaWeights[1][ccIndex];
                        totalNilai += ccContribution;
                }

                if (fuelIndex >= 0) {
                        double fuelContribution = subcriteriaWeights[2][fuelIndex];
                        totalNilai += fuelContribution;
                }

                if (designIndex >= 0) {
                        double designContribution = subcriteriaWeights[3][designIndex];
        ...
        private int getPriceIndex(String kategori) {
                if (kategori.contains("EKONOMIS"))
                        return 0;
                if (kategori.contains("MENENGAH"))
                        return 1;
                if (kategori.contains("PREMIUM"))
                        return 2;
                return -1;
        }

        private int getCCIndex(String kategori) {
                if (kategori.contains("Kecil") || kategori.contains("Entry"))
                        return 0;
                if (kategori.contains("Sedang") || kategori.contains("Mid-range"))
                        return 1;
                if (kategori.contains("Besar") || kategori.contains("Premium"))
                        return 2;
                return -1;
        }

        private int getFuelIndex(String kategori) {
                if (kategori.contains("Irit"))
                        return 0;
                if (kategori.contains("Sedang") && kategori.contains("40"))
                        return 1;
                if (kategori.contains("Boros"))
                        return 2;
                return -1;
        }

        private int getDesignIndex(String kategori) {
                if (kategori.contains("Sporty") || kategori.contains("Agresif"))
                        return 0;
                if (kategori.contains("Retro") || kategori.contains("Stylish"))
                        return 1;
                if (kategori.contains("Futuristik") || kategori.contains("Modern"))
                        return 2;
                return -1;
        }
        ...
        private void SimpanActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_SimpanActionPerformed
                ...
                try {
                        ...
                        // Save hasil_penilaian as double (with 3 decimals for display only)
                        double hasilPenilaianValue = Double.parseDouble(TotalNilai.getText().replace(",", "."));
                        if (exists) {
                                ...
                                updateStat.setDouble(3, hasilPenilaianValue);
                                ...
                                insertStat.setDouble(4, hasilPenilaianValue);
